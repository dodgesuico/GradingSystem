            ...
            <div class="modal-content">
                <div class="modal-header">
                    <h2>Edit Class - {{ $class->subject_code }}</h5>
                        <button type="button" class="close"
                            onclick="closeEditClassModal({{ $class->id }})">&times;</button>
                </div>
                <form action="{{ route('classes.update', $class->id) }}" method="POST">
                    ...
                    <div class="modal-body">
                        <div class="info-container">
                            ...
                            <input type="text" class="form-control" id="subject_code" name="subject_code"
                                value="{{ $class->subject_code }}" required>
                        </div>
                        <div class="info-container">
                        ...
                        <div class="info-container">
                            ...
                            <input type="text" id="instructorSearch{{ $class->id }}" class="form-control"
                                ...
                                oninput="filterInstructorsEdit({{ $class->id }})" value="{{ $class->instructor }}"
                                required>

                            <!-- Hidden input to store selected instructor -->
                            <input type="hidden" id="instructorHidden{{ $class->id }}" name="instructor"
                                value="{{ $class->instructor }}">

                            <div id="instructorDropdown{{ $class->id }}" class="dropdown-menu"></div>
                        </div>
                        ...
                        <div class="info-container">
                            ...
                            <select id="academic_period" name="academic_period">
                                ...
                                <option value="1st Semester"
                                    {{ isset($class) && $class->academic_period == '1st Semester' ? 'selected' : '' }}>
                                    1st Semester</option>
                                <option value="2nd Semester"
                                    {{ isset($class) && $class->academic_period == '2nd Semester' ? 'selected' : '' }}>
                                    2nd Semester</option>
                                <option value="Summer"
                        ...
                        <div class="info-container">
                            ...
                            <input type="text" class="form-control" id="schedule" name="schedule"
                                value="{{ $class->schedule }}" required>
                        </div>

                        <div class="info-container">
                            <label for="status">Status</label>
                            <select class="form-control" id="status" name="status" required>
                                <option value="Active" {{ $class->status == 'Active' ? 'selected' : '' }}>Active
                                </option>
                                <option value="Ended" {{ $class->status == 'Ended' ? 'selected' : '' }}>Ended</option>
                                <option value="Dropped" {{ $class->status == 'Drop' ? 'selected' : '' }}>Dropped
                                </option>
                            </select>
                        </div>
                    ...
                    <div class="modal-footer">
                        ...
                        <button type="submit" class="save-btn"><i class="fa-solid fa-floppy-disk"></i> Save
                            Changes</button>
                        <button type="button" class="btn btn-secondary"
                            onclick="closeEditClassModal({{ $class->id }})">Close</button>
        ...
        <script>
            function filterInstructorsEdit(classId) {
                let input = document.getElementById("instructorSearch" + classId).value.toLowerCase();
                let dropdown = document.getElementById("instructorDropdown" + classId);
                ...
                filtered.forEach(instructor => {
                    ...
                    option.onclick = function() {
                        document.getElementById("instructorSearch" + classId).value = instructor.name;
                        document.getElementById("instructorHidden" + classId).value = instructor.name;
                        dropdown.style.display = "none";
                    };
        ...
        <div id="deleteClassModal{{ $class->id }}" class="modal">
            <div class="modal-content">
                ...
                <div
                    ...
                    <input type="checkbox" id="confirmDelete{{ $class->id }}"
                        onchange="toggleDeleteButton({{ $class->id }})">
                    <label style="color:var(--color6);" for="confirmDelete{{ $class->id }}">I am sure to delete
                        this</label>
                </div>
                ...
                <div class="modal-footer">
                    ...
                    <form method="POST" action="{{ route('classes.destroy', $class->id) }}">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="delete-btn disabled-btn" id="deleteBtn{{ $class->id }}"
                            disabled>
